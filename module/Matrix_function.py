...
def CNN_test(st1, st2, sp, v1, v2, tick, DetPos, table, NowOpen, model_CNN):
    ...
    AllCharInput[:, :2] = (
        AllCharInput[:, :2] * stock_std[:, :2] / np.expand_dims(stock_std[:, 2], axis=1)
    )
...
def fore_chow(stock1, stock2, model, Flen, give=False, p=0, A=0, ut=0, maxp=5):
    ...
    for i in range(1, h + 1):
        ...
        if p == 1:
            ut_h.append(
                lag_mat[0].T - (A[0].T + np.dot(A[1 : k * p + 1].T, lag_mat[1:2].T)).T
            )
        else:
            ut_h.append(
                lag_mat[0].T
                - (
                    A[0].T
                    + np.dot(
                        A[1 : k * p + 1].T, lag_mat[1 : k * p - 1].reshape([k * p, 1])
                    )
                ).T
            )
    ...
    try:
        ...
        if tau_h > float(
            f.ppf(0.99, k, n - k * p + 1)
        ):  # tau_h > float(chi2.ppf(0.99,k)):
