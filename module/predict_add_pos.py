...
class Predictor:
    ...
    def open_Quotes_setting(self, ref_trade_price, target_trade_price,timestamp):
        ...
        self.ref_size = self.table["w1"] * self.table["capital"] / ref_trade_price
        self.target_size = self.table["w2"] * self.table["capital"] / target_trade_price
        ...
        self.logger.fills('BTSE', str(timestamp), self.ref_symbol,
                           'LIMIT', self.side_determination(self.ref_size), ref_trade_price, abs(self.ref_size))
        self.logger.fills('BTSE', str(timestamp), self.target_symbol,
    ...
    def close_Quotes_setting(self, ref_trade_price, target_trade_price,timestamp):
        ...
        self.logger.fills('BTSE', str(timestamp), self.ref_symbol,
                           'LIMIT', CLOSE_POSITION[self.side_determination(self.close_ref_size)], ref_trade_price, abs(self.close_ref_size))
        self.logger.fills('BTSE', str(timestamp), self.target_symbol,
                           'LIMIT', CLOSE_POSITION[self.side_determination(self.close_target_size)], target_trade_price, abs(self.close_target_size))
        self.close_ref_size, self.close_target_size = 0, 0
        self.position = 0
            ...
            if self.position == 0 and self.cointegration_check is False and self.timestamp_check is True:
                ...
                if self.position == -1:
                    if self.ref_size < 0 and self.target_size > 0:
                        ...
                        np.log(ref_ask) + \
                        self.table["w2"] * np.log(target_ask)
                    if spread_stamp_up > (open_threshold + self.increase) * self.table['stdev'] + self.table['mu'] and spread_stamp_up < self.table["mu"] + self.table["stdev"] * stop_loss_threshold and self.increase < ADD_POS * 5:
                        self.increase += ADD_POS
                        if self.table['w1'] < 0 and self.table['w2'] > 0:
                            self.open_Quotes_setting(ref_ask, target_bid,t)
                            print(ref_ask, target_bid)
                    ...
                    if self.ref_size < 0 and self.target_size > 0:
                        ...
                        np.log(ref_bid) + \
                        self.table["w2"] * np.log(target_bid)
                    if spread_stamp_down < self.table['mu'] - (open_threshold + self.increase) * self.table['stdev'] and spread_stamp_down > self.table["mu"] - self.table["stdev"] * stop_loss_threshold and self.increase < ADD_POS * 5:
                        print(self.increase)
                        self.increase += ADD_POS
                        if self.table["w1"] < 0 and self.table['w2'] > 0:
                            self.open_Quotes_setting(ref_bid, target_ask,t)
